...
/**
 * @brief Llama a DLL_Product_Make_Empty y borra la DLL a su vez que livera la memoria solicitada para esto.
 *
 * @param this DLL de  customers.
 *
 * @pre this ya existe.
...
bool DLL_Customer_Find(customer_DLL* this,int id ){
	...
	while( inf <= sup ) {
		...
		if( id == this->cursor->id ) { //Caso base 1
#ifdef DEBUG
			fprintf( stderr, "Coincidencia encontrada en : %d\n", centro );
#endif
			return !encontrado;

		} else if( id < this->cursor->id ) {
	...
	if( this->len == 0 ){
		...
		while(this->cursor->id != id && this->cursor != NULL ){
			#ifdef DEBUG
