		...
		switch( op ){
			...
			case 2:
				if( DLL_Customer_Is_Empty( customers_database ) == false ){
					DLL_Customer_Print( customers_database );
					printf("\n\n\t\tDesea consultar la información de un cliente?( 1->YES ||0->NO) $");
					scanf("%d", &op );
					...
					while( op == 1 ){
						...
						if( DLL_Customer_Find( customers_database, id_cliente ) ){
							...
							printf("\n\n\t\tDesea imprimir los Tickets del Cliente %s - %d?( 1->YES ||0->NO) $", customers_database->cursor->name, customers_database->cursor->id );
							scanf("%d", &op );
							while( op<0 || op>1 ){
								printf("\n\n\tAcción no válida, se aceptan números enteros del 0 al 1.");
								printf("\n\n\t\tDesea imprimir los Tickets del Cliente %s - %d?( 1->YES ||0->NO) $", customers_database->cursor->name, customers_database->cursor->id );
						...
						printf("\n\n\t\tDesea consultar la información de otro cliente?( 1->YES ||0->NO) $");
						scanf("%d", &op );
				...
				if( DLL_Product_Is_Empty( warehouse ) == false ){
					...
					if( DLL_Product_Is_Empty( warehouse ) != true ){
						...
						while( op==1 ){
							...
							printf("\n\t\tVenderle otro porducto al mismo Cliente ID=(%d)?( 1->YES ||0->NO) $" );
							scanf("%d", &op );
