...
void DLL_Tickets_Print(ticket_DLL* this){
	if( DLL_Ticket_Is_Empty( this ) ) {
		...
		for( ticket_node* it = this->first; it != NULL; it = it->next ) {
			printf("\n\tNombre del Producto = %s", it->product_name );
			printf("\tCantidad Comprada = %i\n\t|", it->quantityBuyed );
		}
	}
}
 ...
 *
 * @return True si se encuentra una coincidencia o false si no.
 */
bool DLL_Customer_Find( customer_DLL* this, int id ){
	...
	if( this->len == 0 ){
		...
		while( this->cursor != NULL ){//this->cursor->id != id && 
			...
			if( this->cursor->id == id ){
				...
				#ifdef DEBUG
					fprintf( stderr, "Encontrado, saliendo\n" );
				#endif
				
				return 	encontrado;
				#ifdef DEBUG
					fprintf( stderr, "No debería de salir impreso\n" );
				#endif
			}
	...
	#ifdef DEBUG
		fprintf( stderr, "Saliendo con encontrado=(%d)\n\n", encontrado );
	#endif
