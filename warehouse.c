...
int Calcula_cambio( int cambio, int SolCambio[]){//Regresamos el arreglo por referencia
	...
	int candidatos[MAX_DENOMS] = { 1000, 500, 500,200,200,200,100,100,100,50,50,50,50,20,20,20,20,20,10,10,10,10,10,5,5,5,2,2,2,1};//{ 1000, 500, 200, 100, 50, 20, 10, 5, 5, 2, 2, 1 };
...
*
* @param pago_ticket La cantidad de cambio a dar.
* @param num_monedas La cantidad de monedas que se usan para dar el cambio.
* @param SolCambio[] Arreglo oversiced que guarda las monedas del cambio.
*/
void Imprimir_Cambio( int pago_ticket, int num_monedas, int SolCambio[] ){
	
	if( pago_ticket>0 ){
		printf( "\n\tEl cambio de $%d es con billetes y monedas de: ", pago_ticket );
		...
		printf( "\n\tCantidad exacta, no se da cambio." );
	}
...
/**
 * @brief Hace una lineal y busca una coincidencia con el id dado, 
...
void DLL_Product_Print(product_DLL* this) {
	if( DLL_Product_Is_Empty( this ) ) {
		...
		for( product_node* it = this->first; it != NULL; it = it->next ) {
			printf("\n\tID = %i\t|", it->product_id );
			printf("|\t Nombre = %s", it->product_name );
			printf("\tCantidad = %i\t|", it->product_stock );
			printf("|\tPrecio = $%d\n", it->product_price );
		}
...
void DLL_Product_Cursor_Print(product_DLL* this) {
	...
	//printf("\n\tID\t\tNombre\t\tCantidad\t\tPrecio");
	printf("n\tID = %i\t|", this->cursor->product_id );
	printf("|\t Nombre = %s", this->cursor->product_name );
	printf("\tCantidad = %i\t|", this->cursor->product_stock );
