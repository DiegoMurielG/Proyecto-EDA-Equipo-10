...
int main( void ){
	...
	*/
	...
	while( company_operations_time <= 0 ){
		printf("\n\n\tAño no válido, se aceptan números enteros del 1 en adelante.");
		printf("\n\tIngrese el año de inicio de operaciones de su compañía: $");
		scanf("%d", &company_operations_time);
	}
		...
		switch( op ){
			...
			case 2:
				if( DLL_Customer_Is_Empty( customers_database ) == false ){
					...
					while( op<0 || op>1 ){
						printf("\n\n\tOpción no válida, se aceptan números enteros del 0 al 1.");
						printf("\n\n\t\tDesea consultar la información de la base de datos?( 1->YES ||0->NO) $");
						scanf("%d", &op );
					}
					...
					while( op == 1 ){
						...
						if( op==1 ){
							...
							if( DLL_Customer_Find( customers_database, id_cliente ) == true ){
								...
								printf("\n\n\tID %d no encontrado, eliga la opción 1 y vuelva a ingresar el ID del cliente.", id_cliente );						
							}
			...
			case 4:
				if( DLL_Product_Is_Empty( warehouse ) == false ){
					...
					#ifdef DEBUG
						fprintf( stderr, "\nCambio=(%d)\n",pago_ticket );
						fprintf( stderr, "monedaspara dar cambio=(%d)\n",num_monedas );
					#endif
